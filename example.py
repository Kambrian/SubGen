...
# select a resolution limited final mass sample
mass_selection = (sample.m > sample.mAccMin)
...
y, x = myhist(np.log(sample.mAcc[selection]),
              50, weights=sample.weight[selection])
y = y/(x[1]-x[0])  # dN/dlnx
xmid = np.exp(x[:-1]+(x[1]-x[0])/2)
plt.plot(xmid, y, 'r-', label='Infall')
plt.plot(xmid, sample.HOD.A*Host.M*xmid**-
         sample.HOD.alpha, 'r--', label='Infall(Model)')
selection = (sample.R < 1) & (sample.m > sample.mAccMin)
y, x = myhist(np.log(sample.m[selection]), 50,
              weights=sample.weight[selection])
y = y/(x[1]-x[0])
xmid = np.exp(x[:-1]+(x[1]-x[0])/2)
...
plt.plot(xmid, Host.density(xmid*Host.Rv)**2 *
         Host.Rv**3/Host.Ls/LHost, 'g', label='Host')
plt.legend()
plt.loglog()
plt.xlabel(r'$R/R_{200}$')
