...
class SubhaloSample:
    ...
    def __init__(self, M, N=1e4, MMinInfall=1e-5, MMaxInfall=1e-1, Rmax=2, C=None, include_disruption=True, weighted_sample=True):
        ...
        M: host mass in 1e10Msun/h, defined to be the mass enclosing a density of 200*CriticalDensity
        ...
        # expected number of subhaloes per host.
        self.nPred = self.HOD.A*self.Msample * \
            (self.mAccMin**-self.HOD.alpha -
             self.mAccMax**-self.HOD.alpha)/self.HOD.alpha
    ...
    def _lnPDF(self, x):
        ...
        # dM/dlnR=rho*R^3.
        lnPDFR = 3.*lnR+np.log(self.Host.density(np.exp(lnR)*self.Host.Rv))
        return lnPDFmu+lnPDFR

    def assign_mu_R(self, nwalkers=8, nburn=200, plot_chain=True):
        '''run emcee to sample mu and R '''
        nsteps = int(
            self.n/nwalkers+1+nburn)  # one more step to make up for potential round-off in N/nwalkers
        ...
        x0 += (np.random.rand(ndim*nwalkers).reshape(nwalkers, ndim) -
               0.5)*0.1  # random offset, [-0.5,0.5]*0.1
        ...
        if plot_chain:
            ...
            for i in range(ndim):
                ...
                for j in range(nwalkers):
                    plt.plot(list(range(nsteps)), sampler.chain[j, :, i], '.')
                plt.ylabel(labels[i])
                plt.plot([nburn, nburn], plt.ylim(), 'k--')
            plt.xlabel('Step')
            plt.subplot(211)
            plt.title('%d walkers, %d burn-in steps assumed' %
                      (nwalkers, nburn), fontsize=10)
        # ==========extract mu and R===========
        sample = sampler.chain[:, nburn:, :]
        # take the last N entries
        flatchain = sample.reshape([-1, ndim])[-self.n:]
    ...
    def project_radius(self):
        ...
        phi = np.arccos(np.random.rand(self.n)*2-1.)
    ...
    def assign_mass(self):
        ...
        if self.weighted_sample:
            ...
            # uniform distribution between lnmmin and lnmmax
            lnmAcc = np.random.rand(self.n)*(lnmmax-lnmmin)+lnmmin
            self.mAcc = np.exp(lnmAcc)
            self.weight = self.mAcc**-self.HOD.alpha
            # w/sum(w)*Npred, equals to dN/dlnm*[delta(lnm)/N] as N->inf
            self.weight = self.weight/self.weight.sum()*self.nPred
            ...
            # uniform in m**-alpha which is proportional to N
            mAcc = np.random.rand(self.n)*(mmax-mmin)+mmin
    ...
    def assign_annihilation_emission(self, concentration_model='Ludlow'):
        ...
        logC = np.log10(mean_concentration(
            self.mAcc[:self.nSurvive], concentration_model))  # mean
        sigmaLogC = 0.13
        deltaLogC = np.random.normal(0, sigmaLogC, self.nSurvive)  # scatter
        cAcc = 10**(logC+deltaLogC)
        SatHalo = [NFWHalo(self.mAcc[i], cAcc[i])
                   for i in range(self.nSurvive)]
        rt = np.array([SatHalo[i].radius(self.m[i])
                      for i in range(self.nSurvive)])  # truncation radius
        self.L = np.zeros_like(self.m)
        self.L[:self.nSurvive] = np.array([SatHalo[i].luminosity(
            rt[i]) for i in range(self.nSurvive)])  # truncated luminosity
    ...
    def save(self, outfile, save_all=False):
        ...
        if save_all:
            np.savetxt(outfile, np.array([self.R, self.m, self.mAcc, self.weight, self.Rp, self.mStar, self.L]).T,
                       header='R/R200, m/[1e10Msun/h], mAcc/[1e10Msun/h], weight, Rp/R200, mStar/[1e10Msun/h], Luminosity/[(1e10Msun/h)^2/(kpc/h)^3]')
        else:
            np.savetxt(outfile, np.array([self.R, self.m, self.mAcc, self.weight]).T,
                       header='R/R200, m/[1e10Msun/h], mAcc/[1e10Msun/h], weight')
